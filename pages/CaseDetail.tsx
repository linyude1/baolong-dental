...
const RecordEntry: React.FC<{ 
  ...
  return (
    <div className="bg-white dark:bg-slate-800 rounded-[32px] p-6 shadow-sm border border-slate-100 dark:border-slate-700 flex flex-col items-center text-center relative group">
      {/* 操作按钮组 */}
...
export const CaseDetail: React.FC = () => {
  ...
  const patient = useMemo(() => patients.find(p => p.id === caseId), [caseId, patients]);
    ...
    <div className="flex flex-col min-h-screen bg-slate-50 dark:bg-background-dark">
      <header className="sticky top-0 z-[150] bg-white/95 dark:bg-slate-900/95 backdrop-blur-xl border-b border-slate-100 dark:border-slate-800 flex items-center h-14 px-2">
        ...
        <div className="flex-1 text-center">
          <h2 className="text-[17px] font-black text-slate-800 dark:text-slate-100">诊疗详情</h2>
        </div>
      ...
      <main className="flex-1 space-y-10 p-5 pb-32">
        <section className="bg-white dark:bg-slate-800 rounded-[44px] p-8 shadow-sm border border-slate-100 dark:border-slate-700 flex flex-col items-center text-center">
          ...
          <div className="flex gap-3">
            ...
            <span className={`px-5 py-1.5 text-[11px] font-black rounded-full ${patient.status === PatientStatus.COMPLETED ? 'bg-green-500 text-white' : 'bg-[#fdf3e7] text-orange-400'}`}>
              {patient.status}
            </span>
          </div>
        </section>
        ...
        <section className="space-y-6">
          <div className="flex items-center justify-between px-2">
            <h3 className="text-lg font-black text-slate-800 dark:text-slate-100">复诊诊疗记录</h3>
